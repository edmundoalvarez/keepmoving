...
@section('content')
<h2 class="mb-2">Publicar un producto</h2>

@if($errors->any())
<p class="mb-3 text-danger"> Hay errores en la validación. Por favor, corregirlos</p>
@endif

<form class="my-3" action="{{ url('/dashboard/productos/nuevo')}}" method="POST" enctype="multipart/form-data">
                ...
                @foreach ($sizes as $size)
                    <label for="{{$size->name}}">
                        <input 
                            ...
                            value="{{ $size->size_id }}"
                            @checked(in_array($size->size_id, old('sizes', [])))
                        >
                        <span>{{$size->name}}</span>
                    </label>
                @endforeach
