@php
    /* @var \App\Models\Product[] */
@endphp

@extends('layouts.main')

@section('title', 'Dashboard Productos')

@section('content')
    <h2 class="mb-4">Dashboard de productos</h2>
    <div class=" mb-4 buscador">
        <form action="{{ route('products.dashboard')}}" method="get">
            <div class="mb-2">
                <label for="search-name" class="form-label ">Buscador:</label>
                <input 
                    type="text" 
                    id="search-name" 
                    name="search-name" 
                    class="form-control"
                    value="{{ $searchParams['name'] }}"
                >
            </div>
            <div>
                <button type="submit" class="btn btn-secondary">Buscar</button>
                <a class="btn btn-ver-todos" href="<?= route('products.dashboard'); ?>">Ver todos</a>
            </div>
        </form>
    </div>

    <a href="{{ route('product.create.form') }}" class="mt-[60px] btn btn-secondary mb-4">Publicar un nuevo producto</a>

    <div class="products-all mt-[20px]">
        <div class="product-container d-flex align-content-start flex-wrap gap-3">
            @foreach ($products as $product)
                <div class="card" style="width: 18rem;">
                    <div class="flex-shrink-0">
                        <picture>
                        @if ($product->image_small !== null)
                            <img src="{{ asset('/storage/' . $product->image_small) }}" class="card-img-top" alt="{{ $product->image_description }}">   
                        @else
                            <img src="data:image/png;base64," class="card-img-top" alt="...">
                        @endif
                        </picture>
                    </div>

                    <div class="card-body d-flex flex-column flex-wrap gap-2">
                        <h5 class="card-title mb-0"><b>{{$product->name}}</b></h5>

                        <p class="card-text">${{$product->price}}</p>

                        <div class="card-text d-flex flex-column flex-wrap gap-2">
                            @if ($product->color !== null)
                            <p>Color: <b>{{$product->color->name}}</b></p>  
                            @else             
                            <p>Color: <b>No especificado</b></p>  
                            @endif

                            <div class="sizes">
                                <p>Talles: </p>
                                    @forelse ($product->sizes as $size) 
                                        <p class="size"><b>{{$size->name}}</b></p> 
                                    @empty
                                        <p class="no-size">No Especificado</p> 
                                    @endforelse
                            </div>
                        </div>

                        <div class="d-flex buttons-card">
                            <a href="{{ route('product.update.form', ['id' => $product->product_id]) }}" class="btn btn-warning">Editar</a>
                            
                            <a href="{{ route('product.delete.form', ['id' => $product->product_id]) }}" class="btn btn-danger">Eliminar</a>
                        </div>
                    </div>
                </div>
            @endforeach
        </div>  
    </div>
    <div class="paginador">{{$products->links()}}</div>

@endsection

