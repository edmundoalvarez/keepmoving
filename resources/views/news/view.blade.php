@extends('layouts.main')

@section('title', $news->title)
    
@section('content')
    <div class="news-detalle d-flex flex-column">
        <div>
            <div class="flex-shrink-0 mb-4">
                <picture>
                    @if ($news->image !== null)
                        <img src="{{ asset('/storage/' . $news->image) }}" class="card-img-top" alt="{{ $news->image_description }}">   
                    @else
                        <img src="data:image/png;base64," class="card-img-top" alt="...">
                    @endif            
                </picture>
            </div>
            
            <div class="d-flex flex-column gap-2">
                <p class="italic date mb-2">Publicada el: {{$news->created_at->format('d/m/Y H:i')}}</p>

                <h2 class="mb-4 news-title"><b>{{$news->title}}</b></h2>

                <p>{{$news->text}}</p>

                <a class="mt-3 btn btn-secondary" href="<?= url('/noticias/listado'); ?>">Volver a todas las noticias</a>
            </div>
        </div>
    </div>
@endsection
