@php
    /* @var \App\Models\News[] */
@endphp

@extends('layouts.main')

@section('title', 'Dashboard Noticias')

@section('content')
    <h2 class="mb-4">Dashboard de noticias</h2>

    <a href="{{ route('news.create.form') }}" class="btn btn-secondary mb-4">Publicar nueva  noticia</a>

    <div class="d-flex align-content-start flex-wrap gap-4">
    @foreach ($news as $n)
        <div class="card-news">
            <div class="card-news-img flex-shrink-0">
                @if ($n->image !== null)
                    <img src="{{ asset('/storage/' . $n->image_small) }}" class="card-img-top" alt="{{ $n->image_description }}">   
                @else
                    <img src="data:image/png;base64," class="card-img-top" alt="...">
                @endif
            </div>
            
            <div class="card-news-body card-body d-flex flex-column flex-wrap gap-2 justify-content-between">
                <h5 class="card-title mb-0"><b>{{$n->title}}</b></h5>
                <p class="card-text">{{$n->lead}}</p>
                <div class="d-flex buttons-card mt-3">
                    <a href="{{ route('news.update.form', ['id' => $n->news_id]) }}" class="btn btn-warning">Editar</a>
                    <a href="{{ route('news.delete.form', ['id' => $n->news_id]) }}" class="btn btn-danger">Eliminar</a>
                </div>
            </div>
        </div>
    @endforeach
    </div>  
@endsection

